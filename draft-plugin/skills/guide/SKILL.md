...
Phase 5完了後 → イテレーション(機能追加):
  壁打ち → Feature Design Doc → 実装プロンプト生成 → 実装 → System Design Doc更新 → 繰り返し
```
...
| 機能追加後にSystem Design Docを更新しない | 次の壁打ちで古い情報を元に設計してしまう | 実装プロンプトにドキュメント更新指示を組み込む |
...
着想 -> 壁打ち -> docs/features/YYYYMMDD-HHMM_{機能名}.md -> 実装プロンプト -> 実装 -> System Design Doc更新
```
