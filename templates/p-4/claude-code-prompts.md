...
まだ画面やAPIの実装はしない。
{dev起動コマンド} で両方のdev serverが起動し、
docker compose up -d + {DB反映コマンド} が通る状態をゴールとする。
```
...
4. {DB反映コマンド} + {シードコマンド} が通ることを確認

{DB反映コマンド} でテーブル作成、{シードコマンド} でデータ投入、
{DB GUI起動コマンド} でデータが確認できる状態をゴールとする。
```
...
{テスト実行コマンド} が全パス、{lint実行コマンド} がエラー0の状態をゴールとする。
```
