...
Claude Codeで実装
  ...
  1チャットで完結する粒度
    ↓
System Design Doc更新（Claude Code — 同じチャット）
...
| **大** | 新ロール追加、コアフロー変更 | 壁打ちでFeature Design Doc作成 → 設計仕様書も壁打ち内で更新 → Claude Code 複数チャットで実装 → System Design Doc更新 |
...
### System Design Docの更新

実装完了・動作確認後に、System Design Docを現在の実装に合わせて更新する。
System Design Docは「現在の状態」を表すドキュメントなので、実装前ではなく実装後に更新する。
...
Docsを現在の実装に合わせて更新してください:
- API設計セクション（追加/変更したエンドポイント）
- データモデルセクション（スキーマ変更があった場合）
- 画面一覧（画面追加があった場合）

また上記の3点に限らず更新が必要なドキュメントは全て更新してください。
...
- System Design Docは次のイテレーションの壁打ちで入力として使う
- 常に「現在の状態」を反映していないと、Claude.aiが古い情報で設計してしまう
- Feature Design Docは「変更の経緯」（なぜ変えたか）、System Design Docは「現在の状態」（今どうなっているか）という役割分担
...
- **System Design Docの更新は実装プロンプトに組み込む。** 別作業にすると忘れる。プロンプトに含めればClaude Codeが自動で実行する
...
idea -> brainstorm -> features/YYYYMMDD-HHMM_{name}.md -> impl prompt -> Claude Code -> update system-design-doc
```
